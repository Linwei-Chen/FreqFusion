...
class MSDeformAttnPixelDecoder(BaseModule):
    ...
    def forward(self, feats):
        ...
        # dt = encoder_inputs.dtype
        # if dt == torch.float16:
        #     encoder_inputs = encoder_inputs.float()
        #     level_positional_encodings = level_positional_encodings.float()
        #     padding_masks = padding_masks.float()
        #     spatial_shapes = spatial_shapes.float()
        #     reference_points = reference_points.float()
        #     level_start_index = level_start_index.float()
        #     valid_radios = valid_radios.float()
        memory = self.encoder(
        ...
        # memory = memory.permute(1, 2, 0).type(dt)
